...
class ANN:
    ...
    The layer node has a certain weight to proceed to the next layer.
    Truncnorm is used to get the normal distribution of values.
    ...
    def matrice(self):
        ...
        self.win = X.rvs((self.output_nodes, self.input_nodes))
    ...
    networks output is calculated. To get the error of the output 
    the expected output (the label) is subtracted the actual 
    output (the prediction). The error is used to update the 
    "correctness" of the neurons output.
    Finally the first layer vector and the newly 
    calculated vector are multiplied using the dot-product,
    which is added to the weights.
...
"""
ann = ANN(image_size, out_put_nodes, 0.1)
...
while accuracy < 0.93:
  ...
  errors = error/(correct+error)
  accuracy_scores.append(accuracy)
  error_score.append(errors)
...
#plt.plot(xaxis, error_score, label='Error of epoch')
